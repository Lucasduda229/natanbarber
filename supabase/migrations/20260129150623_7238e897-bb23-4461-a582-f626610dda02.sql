...
DROP TRIGGER IF EXISTS block_time_on_appointment ON public.appointments;

-- Drop the old function with CASCADE
DROP FUNCTION IF EXISTS public.block_time_on_appointment() CASCADE;

-- Create improved function that blocks multiple consecutive slots based on service duration
...
BEGIN
  ...
  -- On UPDATE: if status changes to cancelled, unblock all the time slots
  ...
  -- On DELETE: unblock all the time slots
...
-- Create the trigger (AFTER to ensure appointment_services are already inserted)
CREATE TRIGGER block_time_on_appointment
