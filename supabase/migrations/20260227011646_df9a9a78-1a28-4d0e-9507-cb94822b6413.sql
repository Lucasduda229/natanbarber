...
BEGIN
  ...
  IF TG_OP = 'DELETE' THEN
    ...
    IF is_subscription THEN
      -- Check if any selected service is a "corte"
      SELECT EXISTS(
        SELECT 1 FROM services s 
        WHERE (s.id = OLD.service_id OR s.id IN (SELECT aps.service_id FROM appointment_services aps WHERE aps.appointment_id = OLD.id))
        AND lower(s.name) LIKE '%corte%'
      ) INTO has_corte;
  ...
  IF TG_OP = 'INSERT' THEN
    ...
    IF is_subscription THEN
      SELECT EXISTS(
        SELECT 1 FROM services s 
        WHERE (s.id = NEW.service_id OR s.id IN (SELECT aps.service_id FROM appointment_services aps WHERE aps.appointment_id = NEW.id))
        AND lower(s.name) LIKE '%corte%'
      ) INTO has_corte;
